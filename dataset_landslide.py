...
class LandSlide_Dataset(Dataset):
    def __init__(self, eval_length=32, target_dim=7, mode="train", validindex=0, missratio=70):
        ...
        df = pd.read_csv(
            "./data/landslide/02.csv",
            index_col="Datetime",
            parse_dates=True,
        )
        if missratio == 30:
            df_gt = pd.read_csv("./data/landslide/miss30.csv", index_col="Datetime", parse_dates=True)
        elif missratio == 50:
            df_gt = pd.read_csv("./data/landslide/miss50.csv", index_col="Datetime", parse_dates=True)
        else:
            df_gt = pd.read_csv("./data/landslide/miss70.csv", index_col="Datetime", parse_dates=True)
...
def get_dataloader(batch_size, device, validindex=0, missratio=70):
    dataset = LandSlide_Dataset(mode="train", validindex=validindex, missratio=missratio)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=1, shuffle=True
    )
    dataset_test = LandSlide_Dataset(mode="test", validindex=validindex, missratio=missratio)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=1, shuffle=False
    )
    dataset_valid = LandSlide_Dataset(mode="valid", validindex=validindex, missratio=missratio)

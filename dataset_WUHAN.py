...
class WUHAN_Dataset(Dataset):
    def __init__(self, eval_length=32, target_dim=4, mode="train", validindex=0, missratio=70):
        self.eval_length = eval_length
        self.target_dim = target_dim

        path = "./data/WUHAN/02_mean_std.pk"
        factor_path = './data/WUHAN/factor_mean_std.pk'
        with open(path, "rb") as f:
            self.train_mean, self.train_std = pickle.load(f)
        with open(factor_path, 'rb') as f:
            self.factor_mean, self.factor_std = pickle.load(f)
        if mode == "train":
        ...
        if missratio == 30:
            df_gt = pd.read_csv("./data/WUHAN/miss30.csv", index_col="Datetime", parse_dates=True)
        elif missratio == 50:
            df_gt = pd.read_csv("./data/WUHAN/miss50.csv", index_col="Datetime", parse_dates=True)
        else:
            df_gt = pd.read_csv("./data/WUHAN/miss70.csv", index_col="Datetime", parse_dates=True)

        factor_df = pd.read_csv(
            './data/WUHAN/factor.csv',
            index_col="Datetime",
            parse_dates=True,
        )
        ...
        for i in range(len(month_list)):
            ...
            current_factor = factor_df[df.index.month == month_list[i]]
    ...
    def __getitem__(self, org_index):
        ...
        s = {
            ...
            "factor_data": self.factor[c_month][
                           c_index: c_index + self.eval_length
                           ],
...
def get_dataloader(batch_size, device, validindex=0, missratio=70):
    dataset = WUHAN_Dataset(mode="train", validindex=validindex, missratio=missratio)
    train_loader = DataLoader(
        dataset, batch_size=batch_size, num_workers=1, shuffle=True
    )
    dataset_test = WUHAN_Dataset(mode="test", validindex=validindex, missratio=missratio)
    test_loader = DataLoader(
        dataset_test, batch_size=batch_size, num_workers=1, shuffle=False
    )
    dataset_valid = WUHAN_Dataset(mode="valid", validindex=validindex, missratio=missratio)
